...
def auto_discover_models(base_dir, prompt, model_cat):
    """Automatically discover evaluated models from VJEPA results directories."""
    models = set()
    vjepa_dir = f"{base_dir}/results/vjepa/{prompt}/{model_cat}"
    if os.path.exists(vjepa_dir):
        for model_dir in os.listdir(vjepa_dir):
            if os.path.isdir(os.path.join(vjepa_dir, model_dir)):
...
def generate_csv_markdown_table(model_list, csv_results):
    headers = ["Model", "Surprise"]
    md_table = "| " + " | ".join(headers) + " |\n"
    md_table += "| " + " | ".join(["---"] * len(headers)) + " |\n"

    for model in model_list:
        metrics = csv_results.get(model, {})
        surprise_avg = metrics.get('surprise')

        row = [
            model,
            f"{surprise_avg:.4f}" if surprise_avg is not None else "N/A",
        ]
        md_table += "| " + " | ".join(row) + " |\n"
...
def calculate_csv_averages(models, base_dir, model_cat, prompt):
    """Read vjepa_surprise_scores.csv per model and compute average surprise score."""
    results = {}
    for model in models:
        directory_path = f"{base_dir}/results/vjepa/{prompt}/{model_cat}/{model}"
        target_csv = os.path.join(directory_path, 'vjepa_surprise_scores.csv')

        if not os.path.exists(target_csv):
            # Fallback: try to find any csv that contains surprise scores
            csv_candidates = glob.glob(os.path.join(directory_path, '*surprise*.csv'))
            if csv_candidates:
                target_csv = csv_candidates[0]
            else:
                results[model] = {'surprise': None}
                continue

        df = pd.read_csv(target_csv)
        if 'success' in df.columns:
            # Keep only successful rows if present
            df = df[df['success'].astype(str) == 'True']
        surprise_avg = df['surprise_score'].mean() if 'surprise_score' in df.columns else None

        results[model] = {
            'surprise': float(surprise_avg) if surprise_avg is not None else None,
        }
...
def generate_aggregated_markdown_table(models, prompts, all_csv_results):
    """Generate an aggregated markdown table with VJEPA results from all prompts stacked as rows."""
    headers = ["Model/Prompt", "Surprise"]
    md_table = "| " + " | ".join(headers) + " |\n"
    md_table += "| " + " | ".join(["---"] * len(headers)) + " |\n"
    ...
    for model in models:
        ...
        for prompt in prompts:
            ...
            surprise_avg = csv_metrics.get('surprise')

            row = [f"└─ {prompt}"]
            row.append(f"{surprise_avg:.4f}" if surprise_avg is not None else "N/A")
            md_table += "| " + " | ".join(row) + " |\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VJEPA Surprise Scores to Markdown Converter")
    ...
    for prompt in args.prompt:
        ...
        if args.models is None or args.auto_discover:
            print("Auto-discovering models from VJEPA results directories...")
            ...
            if not discovered_models:
                ...
                print(f"Checked VJEPA: {args.base_dir}/results/vjepa/{prompt}/{args.model_cat}")
                continue
        ...
        # Calculate average surprise score from VJEPA results
        print("\nCalculating VJEPA surprise averages...")
        ...
        # Generate Markdown table from VJEPA CSV results
        print("\nGenerating Markdown table from VJEPA results...")
        ...
        print(f"\nVJEPA Results Summary for {prompt}:")
        for model, metrics in csv_results.items():
            label = model_labels.get(model, model)
            surprise_avg = metrics['surprise']
            print(f"{label}:")
            print(f"  Surprise (average) = {surprise_avg:.4f}" if surprise_avg is not None else f"  Surprise (average) = N/A")
    ...
    print(f"\n{'='*80}")
    print("AGGREGATED VJEPA RESULTS ACROSS ALL PROMPTS")
    print(f"{'='*80}")
