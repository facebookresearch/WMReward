    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                sigma_next = self.scheduler.sigmas[i + 1] if i < len(timesteps) - 1 else current_sigma
                    ...
                    if in_guidance_range and rep < n_repeats:
                        ...
                        with torch.enable_grad():
                            ...
                            if save_intermediate and save_dir is not None:
                                fps = int(additional_inputs.get("intermediate_fps", 16)) if additional_inputs else 16
                            ...
                            mode = additional_inputs.get("loss_mode", "max") if additional_inputs is not None else "max"
                            
                            if mode == "max":
                                # Memory-efficient max mode: first pass without gradients to find max chunk
                                print("max mode first pass")
                                chunk_losses_nograd = []
                                chunk_data = []  # Store chunk data for recomputation
                                ...
                                for s in starts:
                                    ...
                                    # Store chunk data for later recomputation
                                    chunk_data.append((s, ctxt_positions, tgt_positions))

                                    def forward_target_nograd(c):
                                        with torch.no_grad():
                                            h = self.vjepa_target_encoder(c)
                                            h = torch.stack([F.layer_norm(hi, (hi.size(-1),)) for hi in h])
                                            return h

                                    def forward_context_nograd(c):
                                        with torch.no_grad():
                                    ...
                                    with torch.no_grad():
                                        h_tokens = forward_target_nograd(chunk)
                                        z_pred = forward_context_nograd(chunk)
                                        z_chunk = z_pred.to(h_tokens.device)
                                        chunk_loss = loss_fn_v2_nograd(z_chunk, h_tokens)
                                        chunk_losses_nograd.append(chunk_loss)

                                # print("chunk_losses_nograd", chunk_losses_nograd)

                                # Find the chunk with maximum loss
                                max_idx = torch.stack(chunk_losses_nograd).argmax().item()
                                max_s, max_ctxt_positions, max_tgt_positions = chunk_data[max_idx]
                                # Get the max chunk fresh from the original tensor to preserve gradients
                                max_chunk = clip_cthw[:, :, max_s:max_s+WSIZE]
                                
                                # Recompute only the max chunk with gradients enabled
                                ...
                                def forward_context(c):
                                    # Enable gradients w.r.t. input chunk while keeping params frozen
                                    z = self.vjepa_encoder(c, max_ctxt_positions)
                                    z = self.vjepa_predictor(z, max_ctxt_positions, max_tgt_positions)
                                    z = F.layer_norm(z, (z.size(-1),))
                                    return z

                                def loss_fn_v2(z, h):
                                    h = apply_masks(h, max_tgt_positions, concat=False)
                                    loss = 1 - F.cosine_similarity(z, h[0], dim=1).mean()
                                    return loss

                                h_tokens = forward_target(max_chunk)
                                z_pred = forward_context(max_chunk)
                                z_chunk = z_pred.to(h_tokens.device)
                                total_loss = loss_fn_v2(z_chunk, h_tokens)
                                
                            else:  # mean mode - original implementation
                                for s in starts:
                                    chunk = clip_cthw[:, :, s:s+WSIZE]
                                    Tchunk = chunk.size(2)
                                    grid_depth = int(Tchunk // tube)
                        ...
                        if (i + shorten_steps) > travel_time[0] and (i + shorten_steps) < travel_time[1]:
                            ...
                            with torch.no_grad():
                                ...
                                h = next_t - current_t                                # step size in t
                                v = (pred_clean - latents) / (1.0 - current_t + 1e-8) # velocity in x_t space
                                delta_base = h * v                                    # solver's move w/o guidance
