...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __init__(
        ...
        print("🚀 Loading torch V-JEPA model for I2V guidance...")
    ...
    def __call__(
        ...
        VIS_MEM = False
        print_gpu_memory(VIS_MEM, info="Start I2V ================================")
        print(f"🎬 STARTING I2V GUIDANCE PIPELINE GENERATION")
        print(f"   Frames: {num_frames}, Steps: {num_inference_steps}, CFG Scale: {guidance_scale}")

        # Disable gradients for all models initially
        ...
        # Memory optimization - move text encoder to CPU
        ...
        # Define DiT forward function for checkpointing
        def dit_forward(latent_model_input, timestep, prompt_embeds, image_embeds, attention_kwargs):
        ...
        # 6. Denoising loop with V-JEPA guidance
        ...
        print(f"\n🔧 I2V GUIDANCE PIPELINE CONFIGURATION:")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents, condition], dim=1).to(transformer_dtype).requires_grad_(True)
                ...
                # Apply V-JEPA guidance if within guidance range
                if perform_guidance:
                    guidance_steps_count += 1
                    print(f"🚀 APPLYING I2V GUIDANCE at step {i} (timestep={t.item():.0f})")
                    with torch.set_grad_enabled(True):
                        ...
                        if visualize:
                            with torch.no_grad():
                                ...
                                export_to_video(save_frame[0], f"./temp/i2v_guidance/guidance_sample_{rep}_{i}.mp4", fps=16)
                    
                        # Get V-JEPA loss using torch implementation
                        ...
                        with torch.enable_grad():
                            ...
                            context_window_size = getattr(self, 'vjepa_context_length', 10)
                        ...
                        print(f'🎯 I2V GUIDANCE STEP {i} (t={t.item():.0f}): Loss={loss.item():.4f}, Grad_norm={grad.norm(2).item():.4f}')
