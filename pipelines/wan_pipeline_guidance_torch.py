...
class WanPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __init__(
        ...
        # Initialize torch V-JEPA model instead of HuggingFace version
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = latents.to(transformer_dtype).requires_grad_(True)         
                timestep = t.expand(latents.shape[0])
                for rep in range(n_travel):
                    # print("======================================================")
                    # print(latent_model_input.requires_grad)
                    # for p in self.transformer.parameters():
                    #     if not p.requires_grad:
                    #         print(1,p.requires_grad)
                    # for p in self.vae.parameters():
                    #     if not p.requires_grad:
                    #         print(2,p.requires_grad)
                    # for p in self.text_encoder.parameters():
                    #     if not p.requires_grad:
                    #         print(3,p.requires_grad)
                    # print("======================================================")
                ...
                if perform_guidance:
                    ...
                    with torch.set_grad_enabled(True):
                        ...
                        # orig_frame already has correct shape [B, C, T, H, W] for torch V-JEPA
