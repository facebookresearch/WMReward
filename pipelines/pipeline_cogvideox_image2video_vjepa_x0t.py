        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                for rep in range(n_repeats+1):
                    ...
                    pred_original_sample = pred_original_sample.to(prompt_embeds.dtype)[:, additional_frames:].requires_grad_(True)

                    # Optionally decode an intermediate full clip for guidance losses that operate in pixel space
                    if in_guidance_range and rep < n_repeats:
                        vae_decode_scale = float(additional_inputs.get("vae_decode_scale", 1.0)) if additional_inputs is not None else 1.0
                        if vae_decode_scale != 1.0:
                            ...
                            # print("lat_bcfhw", lat_bcfhw.shape)
                        ...
                        # Visualize the decoded frames
                        if save_keyframes and save_dir is not None:
                            fps = int(additional_inputs.get("intermediate_fps", 8)) if additional_inputs is not None else 8
                    ...
                    if in_guidance_range and rep < n_repeats:
                        ...
                        grad = torch.autograd.grad(total_loss, pred_original_sample, retain_graph=False, create_graph=False)[0]
                        ...
                        if (i + shorten_steps) >= travel_time[0] and (i + shorten_steps) <= travel_time[1]:
                            ...
                            with torch.no_grad():
                                delta_base= (noise_pred_text - noise_pred_uncond)                    # vanilla solver move                   # solver's move w/o guidance
