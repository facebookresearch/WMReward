    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                next_t = sigma_next / (sigma_next + 1)
                ...
                for rep in range(n_repeats + 1):
                    ...
                    if in_guidance_range and rep < n_repeats:
                        ...
                        with torch.enable_grad():
                            ...
                            # pred_clean = latents - current_sigma * noise_pred # 1, C, T, H, W in Wan2.1
                            # pred_clean = pred_clean.requires_grad_(True)
                            ...
                            if vae_decode_scale != 1.0:
                                ...
                                pred_for_decode = F.interpolate(
                                    ...
                                    scale_factor=(0.5, 0.4, 0.4),
                                    mode="trilinear",
                                    align_corners=False,
                                )
                            # print("pred_for_decode", pred_for_decode.shape)

                            # Preserve gradient flow by avoiding dtype conversion if possible
                            vae_input = pred_for_decode
                            if pred_for_decode.dtype != self.vae.dtype:
                                print(f"Warning: dtype mismatch - pred_clean: {pred_for_decode.dtype}, vae: {self.vae.dtype}")
                                # Only convert if absolutely necessary
                                vae_input = pred_for_decode.to(self.vae.dtype)
                            decoded_full = self.vae.decode(vae_input, return_dict=False)[0].float()
                            # print("decoded_full", decoded_full.requires_grad, decoded_full.shape)

                            # Save intermediate video for visualization
                            if save_intermediate and save_dir is not None:
                                fps = int(additional_inputs.get("intermediate_fps", 8)) if additional_inputs else 8
                            ...
                            mode = additional_inputs.get("loss_mode", "mean") if additional_inputs is not None else "mean"
                        ...
                        if (i + shorten_steps) > travel_time[0] and (i + shorten_steps) < travel_time[1]:
                            ...
                            with torch.no_grad():
                                ...
                                # noise = randn_tensor(pred_clean.shape, generator=generator, device=device, dtype=pred_clean.dtype)
                                # h = next_t - current_t
                                # v = (pred_clean - latents) / (1 - current_t)
                                # latents = latents + h * v
                                # latents = latents + sigma_next * noise
                                

                                # t = torch.clamp(current_t, 0, 1)
                                # grad_xt = t * grad                      # map to x_t space
                                # eps = 1e-8
                                # scale = guidance_lr[i] * (latents.norm(2) / (grad_xt.norm(2) + eps))
                                # print("scale", scale)
                                # latents = latents - scale * grad_xt

                                # -- guidance at current_t --
                                t = torch.clamp(current_t, 0.0, 1.0)
                                grad_xt = t * grad                              # map to x_t space
                                eps = 1e-8
                                scale = guidance_lr[i] * (latents.norm(2) / (grad_xt.norm(2) + eps))
                                latents = latents - scale * grad_xt             # one stable update (you removed the extra term—good)

                                # -- Euler FM step from t_k -> t_{k+1} --
                                h = next_t - current_t                          # negative
                                v = (pred_clean - latents) / (1.0 - t)         # FM velocity using pred_clean at t_k
                                latents = latents + h * v

                                # -- time-travel jitter at the NEW step (use sigma_next) --
                                noise = randn_tensor(
                                    pred_clean.shape, generator=generator, device=device, dtype=pred_clean.dtype
                                )
                                latents = latents + sigma_next * noise
                            ...
                            #     print("current_sigma", current_sigma, current_t)
                            #     latents = latents - guidance_lr[i] * rho * current_t * grad
                            # print("DLO! done!!!!")
                            ...
                            # eps   = 1e-8
                            # gamma = 2.0        # extra late-time ramp; 1–3 works well
                            # cap_pf = (0.5, 2.) # per-frame relative cap
                            # max_rel = 0.2      # cap update ≤ 20% of ||latents||
                            # momentum = 0.9     # set to 0.0 to disable momentum

                            # B = latents.shape[0]
                            # t = torch.clamp(current_t, 0.0, 1.0)

                            # # 1) Map guidance into x_t space and apply ramp
                            # grad_xt = t * grad
                            # grad_xt = (t**gamma) * grad_xt  # gentle extra ramp toward late steps

                            # # 2) Optional temporal momentum (helps flicker)
                            # if 'g_buf' not in locals():
                            #     g_buf = torch.zeros_like(grad_xt)
                            # g_buf = momentum * g_buf + (1.0 - momentum) * grad_xt
                            # guidance = g_buf

                            # # 3) Global RMS normalization (stable across sizes)
                            # def rms(z, dims):
                            #     return z.pow(2).mean(dim=dims, keepdim=True).sqrt()

                            # x_rms = rms(latents, dims=(1,2,3,4))        # [B,1,1,1,1]
                            # g_rms = rms(guidance, dims=(1,2,3,4))       # [B,1,1,1,1]

                            # base_scale = guidance_lr[i] * (x_rms / (g_rms + eps))  # [B,1,1,1,1]

                            # # 4) Per-frame relative cap (keep temporal balance)
                            # # frame RMS over C,H,W
                            # pf_rms = guidance.pow(2).mean(dim=(1,3,4), keepdim=False).sqrt()  # [B,T]
                            # pf_rel = pf_rms / (pf_rms.mean(dim=1, keepdim=True) + eps)        # [B,T]
                            # pf_rel = torch.clamp(pf_rel, cap_pf[0], cap_pf[1])                # [B,T]
                            # scale_t = base_scale * pf_rel[:, None, :, None, None]             # [B,1,T,1,1]

                            # # 5) Update-size clamp (≤ max_rel * ||latents||)
                            # update = scale_t * guidance
                            
                            # upd_rms = rms(update, dims=(1,2,3,4))                    # [B,1,1,1,1]
                            # limit   = max_rel * x_rms                                # [B,1,1,1,1]
                            # shrink  = torch.clamp(limit / (upd_rms + eps), max=1.0)  # [B,1,1,1,1]
                            # update  = update * shrink

                            # 6) Apply
                            # with torch.no_grad():
                            #     latents = latents - update

                            # # keep momentum buffer around across steps
                            # g_buf = g_buf.detach()  # (store in your sampler state)
                            ...
                            with torch.no_grad():
                                # --- base FM Euler step (same as your vanilla sampler) ---

                                delta_base= (noise_pred_cond - noise_pred_uncond)                  # vanilla solver move                   # solver's move w/o guidance

                                # --- guidance grad in x_t (no extra t-scaling) ---
                                g = grad.float()

                                log_grad_spread(g, delta_base, step_i=i, sample_idx=0, topk=5)

                                # ===== Alignment block (directional projection + cosine gating) =====
                                eps = 1e-8
                                sum_dims = (1,2,3,4)  # over C,T,H,W

                                # dot products / norms
                                dot   = (g * delta_base).sum(dim=sum_dims, keepdim=True)         # <g, Δ>
                                b_sq  = (delta_base.pow(2)).sum(dim=sum_dims, keepdim=True)      # ||Δ||^2
                                g_sq  = (g.pow(2)).sum(dim=sum_dims, keepdim=True)               # ||g||^2
                                b_nrm = (b_sq + eps).sqrt()
                                g_nrm = (g_sq + eps).sqrt()

                                # cosine similarity; clamp negatives (cone constraint)
                                cos     = dot / (b_nrm * g_nrm + eps)                            # [-1,1]
                                cos_pos = cos.clamp(min=0.0)

                                # projection onto Δ; disallow pushing *against* Δ
                                proj_coeff = (dot / (b_sq + eps)).clamp(min=0.0)                 # scalar per-sample
                                g_proj = proj_coeff * delta_base                                  # component along Δ

                                # blend: if well-aligned keep g; if misaligned use its projection
                                alpha = cos_pos.pow(2)                                            # [0..1]
                                g_aligned = alpha * g + (1.0 - alpha) * g_proj
                                # ====================================================================

                                # --- ratio-to-base scaling (trust region) ---
                                base_norm = b_nrm
                                g_norm    = (g_aligned.pow(2)).sum(dim=sum_dims, keepdim=True).sqrt() + eps

                                sigma_i  = self.scheduler.sigmas[i]
                                sigma_0  = self.scheduler.sigmas[0]
                                rho_max, gamma = 0.25, 0.8                                        # tune as needed
                                rho = guidance_lr[i] * rho_max * (sigma_i / (sigma_0 + 1e-8))**gamma

                                scale = (rho * base_norm) / g_norm                                # broadcast-safe
                                latents = latents + delta_base - scale * g_aligned

                                # optional logging
                                # ratio = (scale * g_norm) / (base_norm + eps)
                                # mean_cos = cos.mean().item()
                                # print(f"[k={i}] |Δ|={base_norm.mean().item():.2e} |g|={g_norm.mean().item():.2e} "
                                #     f"ratio={ratio.mean().item():.3f} cos={mean_cos:.3f}")
