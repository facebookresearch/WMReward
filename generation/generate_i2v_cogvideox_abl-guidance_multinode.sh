...
#SBATCH --output=./jobs/guid_%A_%a.out
#SBATCH --error=./jobs/guid_%A_%a.err
        ...
        for triplet in "${TRIPLETS[@]}"; do
                ...
                for guidance_range in "${GUIDANCE_RANGES[@]}"; do
                    ...
                    for CFG_SCALE in "${CFG_SCALES[@]}"; do
                        ...
                        for GUIDANCE_FREQUENCY in "${GUIDANCE_FREQUENCIES[@]}"; do
                        echo "Config: Method=$SAMPLE_METHOD, Model=$MODEL_BASE_NAME, Context=$CONTEXT_LENGTH, Stride=$STRIDE, Range=$TRAVEL_TIME, CFG=$CFG_SCALE, LossMode=$LOSS_MODE, VJEPA=$VJEPA_VARIANT, GuidanceFreq=$GUIDANCE_FREQUENCY"
